...
Class asistenteModel{
	...
	public function getUsuarios(){
		$data=$this->db->querySelect("SELECT * FROM Usuarios");
		return $data;
	}
	...
	public function crearUsuario($nombre,$apellido1,$apellido2,$edad,$telefono,$email,$usuario,$contrasena,$palabraClave,$fechaNacimiento,$fechaIngreso,$perfil,$sede,$activo,$ultimo){
		...
		$sql = "INSERT INTO Usuarios(Nombre,Apellido1,Apellido2,Edad,Telefono,Correo,Usuario,Contrasena,Palabra_Clave,Fecha_Nacimiento,Fecha_Ingreso,Id_Perfil,Id_Sede,Activo,Ultimo_Cambio) VALUES ('$nombre','$apellido1','$apellido2','$edad','$telefono','$email','$usuario','$contrasena','$palabraClave','$fechaNacimiento','$fechaIngreso','$perfil','$sede','$activo','$ultimo')";
		if ($this->db->queryNoSelect($sql)) {
			echo '<meta http-equiv="refresh" content="0; URL=\'https://kronosgym.jcdesarrollo.com/?pag=asistente/usuarios\'" />';
		exit();
		} else {
	...
	public function modificarUsuario($id,$nombre,$apellido1,$apellido2,$edad,$telefono,$email,$usuario,$contrasena,$palabraClave,$fechaNacimiento,$fechaIngreso,$perfil,$sede,$activo,$ultimo){
        ...
        $sql = "UPDATE Usuarios SET Nombre='$nombre', Apellido1='$apellido1', Apellido2='$apellido2', Edad=$edad, Telefono='$telefono', Correo='$email', Usuario='$usuario', Contrasena='$contrasena', Palabra_Clave='$palabraClave', Fecha_Nacimiento='$fechaNacimiento', Fecha_Ingreso='$fechaIngreso', Id_Perfil='$perfil', Id_Sede='$sede', Activo='$activo', Ultimo_Cambio='$ultimo' WHERE Id_Usuario='$id'";
		if ($this->db->queryNoSelect($sql)) {
			echo '<meta http-equiv="refresh" content="0; URL=\'https://kronosgym.jcdesarrollo.com/?pag=asistente/usuarios\'" />';
			exit();
		} else {
	...
	public function crearProveedor($nombre,$cedula,$telefono,$email,$direccion,$ultimo){
        ...
        $sql = "INSERT INTO Proveedores(Nombre_Proveedor,Cedula,Telefono,Correo,Direccion,Ultimo_Cambio) VALUES ('$nombre','$cedula','$telefono','$email','$direccion','$ultimo')";
            if ($this->db->queryNoSelect($sql)) {
                echo '<meta http-equiv="refresh" content="0; URL=\'https://kronosgym.jcdesarrollo.com/?pag=asistente/proveedores\'" />';
            exit();
            } else {
	...
	public function crearIngreso($factura,$usuario,$concepto,$metodoPago,$tipoTransaccion,$monto,$fecha,$ultimo,$id){
		...
		if ($this->db->queryNoSelect($sql)) {
			echo '<meta http-equiv="refresh" content="0; URL=\'https://kronosgym.jcdesarrollo.com/?pag=asistente/ingresos\'" />';
			exit();
		} else {
	...
	public function crearEgreso($factura,$usuario,$concepto,$metodoPago,$tipoTransaccion,$monto,$fecha,$ultimo,$id){
		...
		var_dump($sql);
		if ($this->db->queryNoSelect($sql)) {
			echo '<meta http-equiv="refresh" content="0; URL=\'https://kronosgym.jcdesarrollo.com/?pag=asistente/egresos\'" />';
			exit();
		} else {
